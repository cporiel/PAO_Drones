...
def run_sequence(scf, scf2, sequence):
    cf = scf.cf
    cf2 = scf2.cf

    cf.param.set_value('flightmode.posSet', '1')
    cf2.param.set_value('flightmode.posSet', '1')

    for position in sequence:
        print('Setting position {}'.format(position))
        for i in range(50):
            cf.commander.send_setpoint(position[1], position[0], position[3], int(position[2] * 1000))
            cf2.commander.send_setpoint(position[1] + 0.6, position[0] + 0.6, position[3], int(position[2] * 1000))
            time.sleep(0.1)
...
if __name__ == '__main__':
    ...
    with SyncCrazyflie(uri, cf=Crazyflie(rw_cache='./cache')) as scf:
        with SyncCrazyflie(uri2, cf=Crazyflie(rw_cache='./cache')) as scf2:
            ...
            run_sequence(scf, scf2, sequence)
